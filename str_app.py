...
with st.sidebar:
    ...
    if auto_rental_yield is not None:
        st.success(f"✅ Rental Yield  →  {auto_rental_yield:.2f}%")
    elif chosen_locality:
        st.info("ℹ️ Rental Yield  →  Na")
...
def show_result(pred_ratio: float, circle_rate: float, area_sqft: float, model_name: str):
...
with tab_bf:
    ...
    if not bf_model:
        ...
        if predict_btn:
            ...
            show_result(pred_ratio, inputs["circle_rate"], inputs["area_sqft"], "Builder Floor")
...
with tab_apt:
    ...
    if not apt_model:
        ...
        if predict_btn_apt:
            ...
            show_result(pred_ratio_apt, inputs_apt["circle_rate"], inputs_apt["area_sqft"], "Apartment")
