"""
Apartment Model Trainer
=======================
Matches notebooks/notebooks/sell/apt/apartment_model.ipynb exactly.
...
Key improvements over previous version:
  ...
  - locality_tier_map fitted on X_train only (no leakage)
"""
...
# ── Base numeric feature columns (before Voronoi/locality_tier) ─
_BASE_NUMERIC_COLS = [
...
class AptModelTrainer:
    def __init__(
        self,
        data_transformation_artifact: DataTransformationArtifact,
        config: AptModelTrainerConfig = AptModelTrainerConfig(),
    ):
    ...
    def _filter_outliers(df: pd.DataFrame) -> pd.DataFrame:
        """Domain hard-rule outlier removal matching the apartment notebook."""
        mask = (
            df["bhk"].between(1, 6)
    ...
    def _fit_locality_tier_map(
        ...
        Groups by locality, computes median circle_rate per locality,
        then assigns tier based on quantiles across all localities:
          budget  < Q25 <= mid < Q50 <= high < Q75 <= luxury
        Returns {locality_name: tier_str} dict.
        """
        if locality_col not in X_train.columns or "circle_rate" not in X_train.columns:
            return {}

        loc_cr = (
            X_train.groupby(locality_col)["circle_rate"]
            .median()
            .dropna()
        )
        if len(loc_cr) < 4:
    ...
    def _add_locality_tier_features(
        ...
        """Apply locality_tier_map → 4 OHE columns, drop locality column."""
        X = X.copy()
    ...
    def initiate_model_training(self) -> AptModelTrainerArtifact:
        try:
            self._info("============ Apt Model Training Started ============")
            os.makedirs(self.config.model_dir, exist_ok=True)

            # ── 1. Load & filter apartment rows ───────────────
            df_all = pd.read_csv(
            ...
            self._info(f"Apartment rows: {len(df)}")
            if len(df) < 50:
                raise ValueError(f"Too few apartment rows ({len(df)}) to train model.")
            ...
            self._info(
                f"Rows after null drop: {len(df)} (dropped {before - len(df)})"
            )
            ...
            # Include `locality` temporarily for locality_tier fitting after split
            ...
            # Fill numeric nulls only; keep locality as string
            ...
            # ── 13. Locality tier (fit on X_train only) ────────
            locality_tier_map = self._fit_locality_tier_map(X_train)
            ...
            self._info(
                f"Locality tier map fitted: {len(locality_tier_map)} localities"
            )
            ...
            # ── 16. Evaluate — back-calculate to price_per_sqft
            ...
            # ── 18. MLflow logging (non-blocking) ─────────────
            if HAS_MLFLOW:
                try:
                    run_name = (
                    ...
                    with mlflow.start_run(run_name=run_name):
                        ...
                        mlflow.log_params({
                            ...
                            "target":             "log1p_price_by_circle",
                            "n_locality_tiers":   len(locality_tier_map),
                            **best_hp,
                        })
                        # ── Test metrics ──────────────────────────────────
            ...
            # ── 19. TensorBoard deep-dive analysis ──────────
            if HAS_TB_ANALYSIS:
                try:
                    import os as _os
                    tb_dir = _os.path.join(
                        "artifact", "tensorboard", "apt",
            ...
            self._info(f"Saved apt model → {self.config.model_file_path}")
            self._info("============ Apt Model Training Finished ============")

            return AptModelTrainerArtifact(
